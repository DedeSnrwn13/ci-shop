...
class Myorder extends MY_Controller 
{
	...
	public function confirm($invoice)
	{
		...
		if ($data['order']->status !== 'waiting') {
			$this->session->set_flashdata('warning', 'Bukti transfer sudak dikirim');
			base_url(redirect("/myorder/detail/$invoice"));
		}
		...
		if ($this->myorder->create($data['input'])) {
			...
			$this->session->set_flashdata('error', 'Oops! Terjadi keslahan');
		}
